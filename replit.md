...
-   **i18n System (Admin + Manager — 100% complete):** Full internationalization for both admin and manager sections. Dictionaries live in `i18n/` (`types.ts`, `en.ts`, `ru.ts`, `index.ts`). `LanguageProvider` in `context/LanguageContext.tsx` persists locale to `AsyncStorage` under `@app:locale`. `useT()` hook in `hooks/useT.ts` returns the typed `AppDict` for the active locale. `LanguageSelectorSheet` provides flag-and-label EN/RU picker. The AdminHeader renders a flag emoji button (🇬🇧/🇷🇺) that opens the sheet. Every admin and manager screen/component uses `useT()` — zero hardcoded English UI strings remain in either role. Manager sections covered: `_layout.tsx`, `dashboard.tsx`, `services.tsx`, `settings.tsx`, `invoices.tsx`, `notifications.tsx`, `users.tsx`, `patients/[id].tsx`, `doctors/index.tsx`, `hotels/index.tsx`, `transports/index.tsx`, `document-types/index.tsx`, and all manager sub-components. Adding new pages: extend `AppDict` in `i18n/types.ts` + add keys to `en.ts`/`ru.ts`, then call `useT()`.
