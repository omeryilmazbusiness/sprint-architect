...
**Current state (Sprints 1-9 complete + Billing Sprint + Unified Users + Auth Hardening + Credential Request Workflow):** Full multi-tenant Health Tourism SaaS. Invoice status enum migrated to PENDING/UNPAID/PAID. Istanbul-timezone-aware billing scheduler: Job A (09:00 last day of month → generate PENDING invoices + email clinics), Job B (00:00 daily → mark overdue PENDING→UNPAID, suspend clinic), Job C (00:00 day-1 → monthly report email). Email system: nodemailer with DevConsoleEmailProvider (dev) and SmtpEmailProvider (prod). Admin "Mark as PAID" flow: records paidByUserId, reactivates clinic + all users. Admin Users screen shows UNIFIED list of ADMIN + MANAGER + PATIENT entities with type filter chips and status chips. Invoice generation is now fully automated (scheduler only — no manual generate endpoint). `lib/api/adminUsers.ts` exports `listUnifiedEntities()` and `UnifiedEntity` type. `userRepo.listUnified()` merges users table (ADMIN/MANAGER) + patients table with clinic name lookup.

**Credential Request Workflow:** `credentialRequests` table (kind: MANAGER_PASSWORD | GUEST_ACCESS_KEY, status: PENDING | COMPLETED | REJECTED). Public rate-limited endpoint `POST /v1/auth/credential-requests` for users to submit requests. Admin endpoints: `GET /v1/admin/notifications/unread-count` (badge count), `GET /v1/admin/notifications/credential-requests` (list all/pending), `POST /v1/admin/credential-requests/:id/resolve` (generate+email temp password or new guest key), `POST /v1/admin/credential-requests/:id/reject`. Admin dashboard header has notification bell with live badge that polls every 30s. Admin notifications screen at `/(admin)/notifications` shows approve/reject workflow. Login screen has two request modals: "Forgot password?" (Management tab) and "Request a New Access Key" (Guest tab).

**Auth Hardening:** Password policy enforcer (`server/auth/passwordPolicy.ts`) requires 12+ chars, uppercase, lowercase, number, special char, rejects common passwords and email-as-password. `users` schema now has `lastLoginAt` + `lastLoginIp` columns. Admin security endpoints: `POST /v1/admin/auth/change-password` (validates policy, revokes all sessions, audit logs), `POST /v1/admin/auth/logout-all` (revokes all tokens), `GET /v1/admin/audit-logs` (newest-first, limit param). Login now records lastLoginAt + IP + audit log. Rate limiting tightened: 5 attempts/10 min per IP+email for login; 3 attempts/10 min per userId for change-password. Settings screen redesigned as Stack navigator at `app/(admin)/settings/` with Profile (lastLoginAt display), Security (Change Password, Logout All Devices w/ confirmation modal, Sign Out), Administration, System Health + Scheduler status, Audit Log (10 most recent entries).
