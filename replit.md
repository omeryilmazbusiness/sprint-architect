...
-   **Admin i18n System:** Full internationalization for the admin section. Dictionaries live in `i18n/` (`types.ts`, `en.ts`, `ru.ts`, `index.ts`). `LanguageProvider` in `context/LanguageContext.tsx` persists locale to `AsyncStorage` under `@app:locale`. `useT()` hook in `hooks/useT.ts` returns the typed `AppDict` for the active locale. `LanguageSelectorSheet` (same Modal+Animated pattern as `AdminProfileMenu`) provides flag-and-label EN/RU picker. The AdminHeader renders a flag emoji button (🇬🇧/🇷🇺) that opens the sheet. Admin dashboard fully localized — all section labels, action rows, and error strings respect the selected locale. Adding new pages: extend `AppDict` in `i18n/types.ts` + add keys to `en.ts`/`ru.ts`, then call `useT()`.
